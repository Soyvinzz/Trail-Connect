...
<div class="container container--app container--narrow">
    ...
    <form class="ce-form card card--stack glass-stack" method="post" action="index.php?page=create_event">
        ...
        <fieldset class="ce-panel">
            ...
            <h3 class="ce-panel__title" style="margin-top:1rem">Joiner requirements</h3>
            <label class="field-label" for="evt-min-level">Minimum hiking level</label>
            <select id="evt-min-level" class="input input--select" name="min_hiking_level">
                <option value="" <?php echo $minHikingLevel === '' ? 'selected' : ''; ?>>No minimum level</option>
                <option value="beginner" <?php echo $minHikingLevel === 'beginner' ? 'selected' : ''; ?>>Beginner</option>
                <option value="minor" <?php echo $minHikingLevel === 'minor' ? 'selected' : ''; ?>>Minor hikes level</option>
                <option value="intermediate" <?php echo $minHikingLevel === 'intermediate' ? 'selected' : ''; ?>>Intermediate</option>
                <option value="advanced" <?php echo $minHikingLevel === 'advanced' ? 'selected' : ''; ?>>Advanced</option>
            </select>

            <label class="field-label" for="evt-minor-min">Minimum minor hikes completed</label>
            <input id="evt-minor-min" class="input" type="number" min="0" step="1" name="min_minor_hikes" value="<?php echo (int) $minMinorHikes; ?>">

            <label class="field-label" for="evt-major-min">Minimum major hikes completed</label>
            <input id="evt-major-min" class="input" type="number" min="0" step="1" name="min_major_hikes" value="<?php echo (int) $minMajorHikes; ?>">
        </fieldset>
